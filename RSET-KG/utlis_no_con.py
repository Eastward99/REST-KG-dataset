...
def find_optimal_k(elbow_range, data):
    sse = []  # 存储不同 k 值对应的 SSE
    silhouette_scores = []  # 存储不同 k 值对应的轮廓系数

    for k in elbow_range:
        kmeans = KMeans(n_clusters=k, random_state=42)
        kmeans.fit(data)
        sse.append(kmeans.inertia_)  # 计算簇内误差平方和
        silhouette_scores.append(silhouette_score(data, kmeans.labels_))  # 计算轮廓系数

    # 绘制肘部法结果
    plt.figure(figsize=(10, 5))
    plt.subplot(1, 2, 1)
    plt.plot(elbow_range, sse, marker='o')
    ...
    # 绘制轮廓系数图
    plt.subplot(1, 2, 2)
    plt.plot(elbow_range, silhouette_scores, marker='o')
    ...
    # 返回最佳 k 值，选择具有最大轮廓系数的 k
    optimal_k = elbow_range[np.argmax(silhouette_scores)]
    print(f"Optimal number of clusters based on silhouette score: {optimal_k}")
    return optimal_k
def graph_reconstruction_loss(pred_adj_matrix, true_adj_matrix):
    return F.mse_loss(pred_adj_matrix, true_adj_matrix)
...
def encoding_data(data):
    ...
    data['IndustryAdded_id'] = IndustryAdded_encoder.fit_transform(data['产业增加值指标名称']) + 14 * len(data['city_id']) + year_long
    data['IncomeSavings_id'] = IncomeSavings_encoder.fit_transform(data['收入储蓄指标名称']) + 15*len(data['city_id']) + year_long
    data['FiscalTaxation_id'] = FiscalTaxation_encoder.fit_transform(data['财政税收指标名称']) + 16*len(data['city_id']) + year_long
    data['Exit_id'] = Exit_encoder.fit_transform(data['出口指标名称']) + 17*len(data['city_id']) + year_long
    data['AgriculturalOutput_id'] = AgriculturalOutput_encoder.fit_transform(data['农业产量指标名称']) + 18*len(data['city_id']) + year_long
    data['Investment_id'] = Investment_encoder.fit_transform(data['投资指标名称']) + 19*len(data['city_id']) + year_long
    data['Consumption_id'] = Consumption_encoder.fit_transform(data['消费指标名称']) + 20*len(data['city_id']) + year_long
...
def concat_edge(data,node_id,city_connection_path):
    ...
    city_IndustryAdded_edges = torch.tensor([[node_id_map[time_id] for time_id in data['IndustryAdded_id'].values],[node_id_map[city_id] for city_id in data['city_id'].values]],dtype=torch.long)
    ...
    city_indicator_edges = torch.cat([city_UrbanArea_edges,city_AdministrativeUnits_edges, city_CulSpoFacilities_edges,
                                      ...
                                      city_GDP_edges, city_IndustryAdded_edges,city_IncomeSavings_edges,
                                      city_FiscalTaxation_edges, city_Exit_edges,city_AgriculturalOutput_edges, city_Investment_edges,
                                      city_Consumption_edges
                                      ], dim=1)
    ...
    city_city_edges = torch.tensor([[node_id_map[join_id] for join_id in final_df_replaced['Join'].values],
                                    [node_id_map[target_id] for target_id in final_df_replaced['Target'].values]],dtype=torch.long)
    ...
    #indicator_indicator_de_edges = torch.cat([AgriculturalLand_GDP_edges,Climate_AgriculturalOutput_edges,Climate_IndustryAdded_edges,Climate_GDP_edges,Climate_Consumption_edges], dim=1)

    # 合并 edge_index########################################################################################################
    indicator_city_edges = city_indicator_edges.flip(dims=[0])
    time_county_edges = county_time_edges.flip(dims=[0])

    edge_index = torch.cat([county_time_edges, time_county_edges, city_indicator_edges,indicator_city_edges,city_city_edges,time_time_edges,indicator_indicator_in_edges,indicator_indicator_de_edges], dim=1)
    ...
    edge_type = torch.cat([
        ...
        torch.ones(time_county_edges.size(1), dtype=torch.long),  # has_indicator 关系
        torch.full((city_indicator_edges.size(1),), 2, dtype=torch.long),  # 指标节 点类型为 2
        torch.full((indicator_city_edges.size(1),), 3, dtype=torch.long),  # 指标节 点类型为 2
        torch.full((city_city_edges.size(1),), 4, dtype=torch.long),  # 指标节 点类型为 2
        torch.full((time_time_edges.size(1),), 5, dtype=torch.long),  # 指标节 点类型为 3
        torch.full((indicator_indicator_in_edges.size(1),), 4, dtype=torch.long),  # 指标节 点类型为 4
        torch.full((indicator_indicator_de_edges.size(1),), 5, dtype=torch.long),  # 指标节 点类型为 5
    ])
    return node_id_map,edge_index,edge_type


def concat_node(data,city_features,time_features,UrbanArea_features,AdministrativeUnits_features,CulSpoFacilities_features,AgriculturalLand_features,PopSize_features,Practitioner_features,
                MedicalResources_features,SocialWelfare_features,CommunicationUsers_features,EducationalResources_features,EnergyConsumption_features,EmissionsPollution_features,GDP_features,
                IndustryAdded_features,IncomeSavings_features,FiscalTaxation_features,Exit_features,AgriculturalOutput_features,Investment_features,Consumption_features):
    ...
    IndustryAdded_features_normal = normalization(IndustryAdded_features)
    ...
    feature_dimensions = [
        ...
        IndustryAdded_features_normal.shape[1],IncomeSavings_features_normal.shape[1],FiscalTaxation_features_normal.shape[1],
        Exit_features_normal.shape[1],AgriculturalOutput_features_normal.shape[1],Investment_features_normal.shape[1],
        Consumption_features_normal.shape[1]]
    ...
    IndustryAdded_features_padded = pad_features_to_position(IndustryAdded_features_normal, start_positions[15],total_feature_dim)
    IncomeSavings_features_padded = pad_features_to_position(IncomeSavings_features_normal, start_positions[16],total_feature_dim)
    FiscalTaxation_features_padded = pad_features_to_position(FiscalTaxation_features_normal, start_positions[17],total_feature_dim)
    Exit_features_padded = pad_features_to_position(Exit_features_normal, start_positions[18], total_feature_dim)
    AgriculturalOutput_features_padded = pad_features_to_position(AgriculturalOutput_features_normal,start_positions[19], total_feature_dim)
    Investment_features_padded = pad_features_to_position(Investment_features_normal, start_positions[20],total_feature_dim)
    Consumption_features_padded = pad_features_to_position(Consumption_features_normal, start_positions[21], total_feature_dim)
    ...
    node_features = torch.cat((city_features_padded,time_features_padded,UrbanArea_features_padded,AdministrativeUnits_features_padded,
        ...
        EnergyConsumption_features_padded,EmissionsPollution_features_padded,GDP_features_padded,IndustryAdded_features_padded,
        IncomeSavings_features_padded,FiscalTaxation_features_padded,Exit_features_padded,AgriculturalOutput_features_padded,
        Investment_features_padded,Consumption_features_padded
    ...
    node_id16 = torch.cat([torch.tensor(data['IndustryAdded_id'].values)]).unique()
    node_id17 = torch.cat([torch.tensor(data['IncomeSavings_id'].values)]).unique()
    node_id18 = torch.cat([torch.tensor(data['FiscalTaxation_id'].values)]).unique()
    node_id19 = torch.cat([torch.tensor(data['Exit_id'].values)]).unique()
    node_id20 = torch.cat([torch.tensor(data['AgriculturalOutput_id'].values)]).unique()
    node_id21 = torch.cat([torch.tensor(data['Investment_id'].values)]).unique()
    node_id22 = torch.cat([torch.tensor(data['Consumption_id'].values)]).unique()
    ...
    node_id = torch.cat([node_id1,node_id2,node_id3,node_id4,node_id5,node_id6,
                         ...
                         node_id19, node_id20, node_id21,node_id22])
    ...
    node_type = torch.cat([
        ...
        torch.full((IndustryAdded_features.size(0),), 15, dtype=torch.long),  # 指标节 点
        torch.full((IncomeSavings_features.size(0),), 16, dtype=torch.long),  # 指标节 点
        torch.full((FiscalTaxation_features.size(0),), 17, dtype=torch.long),  # 指标节 点
        torch.full((Exit_features.size(0),), 18, dtype=torch.long),  # 指标节 点
        torch.full((AgriculturalOutput_features.size(0),), 19, dtype=torch.long),  # 指标节 点
        torch.full((Investment_features.size(0),), 20, dtype=torch.long),  # 指标节 点
        torch.full((Consumption_features.size(0),), 21, dtype=torch.long)
    ])
...
def graph_make(data_path):
    ...
    # 获取城市属性特征（经纬度、所属省份等）
    city_features = torch.tensor(data[['province_id','经度', '纬度','行政区域土地面积(平方公里)']].values,dtype=torch.float)
    ...
    AdministrativeUnits_features = torch.tensor(data[['乡及镇个数(个)', '乡个数(个)', '镇个数(个)', '街道办事处个数(个)', '村民委员会个数(个)']].values,dtype=torch.float)
    CulSpoFacilities_features = torch.tensor(data[['艺术表演场馆数_剧场、影剧院(个)', '公共图书馆总藏量(千册)', '体育场馆机构数(个)']].values,dtype=torch.float)
    AgriculturalLand_features = torch.tensor(data[['农作物总播种面积(千公顷)', '常用耕地面积(公顷)', '机收面积(公顷)', '设施农业占地面积(公顷)']].values,dtype=torch.float)

    # 人口指标
    PopSize_features = torch.tensor(data[['年末总户数(户)', '乡村户数(户)', '年末总人口(万人)', '乡村人口(万人)', '户籍人口数(万人)']].values,dtype=torch.float)
    ...
    # 经济指标
    GDP_features = torch.tensor(data[['地区生产总值(万元)', '农林牧渔业总产值(万元)']].values,dtype=torch.float)
    IndustryAdded_features = torch.tensor(data[['第一产业增加值(万元)', '第二产业增加值(万元)', '第三产业增加值(万元)', '工业增加值(万元)',
                                                '农业增加值(万元)', '牧业增加值(万元)']].values,dtype=torch.float)
    IncomeSavings_features = torch.tensor(data[['城镇单位在岗职工总工资(元)', '城乡居民储蓄存款余额(万元)', '年末金融机构各项贷款余额(万元)']].values,dtype=torch.float)
    FiscalTaxation_features = torch.tensor(data[['地方财政一般预算收入(万元)', '各项税收(万元)', '地方财政一般预算支出(万元)']].values,dtype=torch.float)
    ...
    EnergyConsumption_features,EmissionsPollution_features,GDP_features,IndustryAdded_features,IncomeSavings_features,FiscalTaxation_features,Exit_features,AgriculturalOutput_features,
    Investment_features,Consumption_features)
    ...
    return node_features,node_id,node_type,node_id_map, edge_index, edge_type, total_feature_dim


#
# node_features,node_id,node_type,node_id_map, edge_index, edge_type,total_feature_dim = graph_make('./data/grouped_data_filtered0121.csv')
# print('end')